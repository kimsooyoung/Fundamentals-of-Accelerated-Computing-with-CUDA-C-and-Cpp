...
int main()
{
  ...
  mallocErr = cudaMallocManaged(&a, size);
    
  if (mallocErr != cudaSuccess)                           // `cudaSuccess` is provided by CUDA. 
  {
    printf("Error: %d %s\n", mallocErr, cudaGetErrorString(err)); // `cudaGetErrorString` is provided by CUDA.
  }
...
//   size_t threads_per_block = 1024;
  ...
  if (syncErr != cudaSuccess) printf("Error: %d %s\n",syncErr, cudaGetErrorString(syncErr));
